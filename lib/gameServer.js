...
class TurfLootGameRoom {
  ...
  async addPlayer(socket, userInfo) {
    ...
    // CHANGED: For persistent practice room, immediately join ongoing match
    if (this.id === 'global-practice-bots') {
      player.ready = true // Automatically ready for practice mode
      player.entryFeePaid = true // No payment needed for practice
      ...
      socket.emit('joined', { 
        ...
        fee: this.fee,
        playerId: socket.id,
        matchInProgress: true, // NEW: Indicate joining match in progress
        instantJoin: true // NEW: No waiting period
      })
      
      // Immediately notify player that they're in an active match
      ...
      console.log(`🎮 Player ${player.nickname} joined ongoing practice match`)
    } else {
      // Regular behavior for other rooms
      socket.emit('joined', { 
        roomId: this.id, 
        mode: this.mode, 
        fee: this.fee,
  ...
  removeSpectator(socketId) {
    ...
    if (spectator) {
      ...
      // If no spectators left, clean up if needed
      if (this.spectators.size === 0 && this.players.size === 0) {
        console.log('👁️ No spectators or players left, cleaning up room')
      }
  ...
  // Removed bot spawning logic - this is now pure multiplayer
  ...
  broadcastState() {
    // Add null check for io instance
    if (!this.io) {
      console.warn(`⚠️ Cannot broadcast state for room ${this.id} - Socket.IO not initialized`)
      return
    }
  ...
  broadcastRoomInfo() {
    // Add null check for io instance
    if (!this.io) {
      console.warn(`⚠️ Cannot broadcast room info for room ${this.id} - Socket.IO not initialized`)
      return
    }
  ...
  async checkWinCondition() {
    ...
    // Add null check for io instance
    if (!this.io) {
      console.warn(`⚠️ Cannot check win condition for room ${this.id} - Socket.IO not initialized`)
      return
    }

    const alivePlayers = [...this.players.values()].filter(p => p.alive)
    
    // CHANGED: For persistent practice room, don't end the match - keep it running
    if (this.id === 'global-practice-bots') {
      // In practice mode, always keep running even if no players
      return
    }
...
export class TurfLootGameServer {
  ...
  initializePersistentServers() {
    ...
    // REMOVED: Don't call initializePersistentPracticeRoom() here - wait for Socket.IO to be ready
  ...
  // NEW: Initialize the persistent practice room that's always running
  initializePersistentPracticeRoom() {
    const practiceRoomId = 'global-practice-bots'
    console.log('🚀 Starting persistent practice room...')
    
    // Create the room immediately
    const practiceRoom = new TurfLootGameRoom(this.io, practiceRoomId, 'practice', 0)
    this.rooms.set(practiceRoomId, practiceRoom)
    
    // Start the room immediately with bots (no need to wait for players)
    practiceRoom.running = true
    practiceRoom.started = true
    practiceRoom.startTime = new Date()
    
    // Always keep bots running in the practice room
    this.startPersistentBots(practiceRoom)
    
    console.log('✅ Persistent practice room is now live and running!')
    console.log(`🎮 Room: ${practiceRoomId} | Mode: practice | Status: always running`)
  }

  // NEW: Start and maintain persistent bots in the practice room
  startPersistentBots(room) {
    // ALWAYS spawn and maintain bots in practice room (even when empty)
    room.spawnBotsForSpectators() // Initial bot spawning
    
    console.log('🤖 Spawning persistent bots for always-active practice arena')
    
    // Set up persistent bot management - bots run 24/7
    const botUpdateInterval = setInterval(() => {
      // CHANGED: Always keep bots active, regardless of player/spectator count
      if (!room.virtualBots || room.virtualBots.size === 0) {
        // Respawn bots immediately if they don't exist
        room.spawnBotsForSpectators()
        console.log('🤖 Respawned bots in persistent practice room')
      } else {
        // Update existing bots
        room.updateVirtualBots()
        
        // Send game state to any connected players/spectators
        if (room.players.size > 0 || room.spectators.size > 0) {
          room.sendSpectatorGameState()
        }
      }
    }, 1000 / 30) // 30 FPS bot updates
    
    // Store interval reference for cleanup if needed
    room.botUpdateInterval = botUpdateInterval
    
    console.log('🤖 Persistent bots started - arena will ALWAYS have active bots for practice')
  }
  ...
  initialize(server) {
    ...
    // NEW: Initialize persistent practice room AFTER Socket.IO is ready
  ...
  // HATHORA INTEGRATION: Room management for Hathora
  getHathoraRoomState(roomId) {
    const room = this.rooms.get(roomId)
    if (!room) return null
    ...
    return {
      ...
      gameState: {
        bots: room.virtualBots ? room.virtualBots.size : 0,
        food: room.food ? room.food.length : 0,
        running: room.running,
  ...
  // HATHORA INTEGRATION: Clean up room for Hathora
  cleanupHathoraRoom(roomId) {
    const room = this.rooms.get(roomId)
    if (room) {
      // Clear intervals
      ...
      console.log(`🗑️ Cleaned up Hathora room: ${roomId}`)
    }
  }
}
