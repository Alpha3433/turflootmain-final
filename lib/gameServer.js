...
export class TurfLootGameServer {
  ...
  startPersistentBots(room) {
    // Spawn initial bots for ongoing gameplay
    room.spawnBotsForSpectators() // This method already exists
    
    // Set up persistent bot management
    setInterval(() => {
      if (room.virtualBots && room.virtualBots.size > 0) {
        // Update bots and keep them active
        room.updateVirtualBots()
        
        // Send game state to any connected players/spectators
        room.sendSpectatorGameState()
      } else if (room.players.size > 0 || room.spectators.size > 0) {
        // Respawn bots if none exist but players/spectators are present
        room.spawnBotsForSpectators()
      }
    }, 1000 / 30) // 30 FPS bot updates
    
    console.log('🤖 Persistent bots started - room will always have active gameplay')
  }
