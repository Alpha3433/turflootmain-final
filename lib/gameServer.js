...
class TurfLootGameRoom {
  ...
  async addPlayer(socket, userInfo) {
    ...
    // ENHANCED DEBUGGING for Global Multiplayer
    if (this.id === 'global-practice-bots') {
      console.log(`🌍 GLOBAL MULTIPLAYER JOIN: ${player.nickname} (${socket.id}) joining shared room`)
      console.log(`🎮 Current players in global room: ${this.players.size}`)
      console.log(`🌐 All player nicknames: ${Array.from(this.players.values()).map(p => p.nickname).join(', ')}`)
    }
    ...
    if (this.id === 'global-practice-bots') {
      ...
      socket.emit('joined', { 
        ...
        totalPlayersInRoom: this.players.size // DEBUGGING: Show total players
      })
      ...
      socket.emit('match_start', {
        ...
        roomInfo: {
          ...
          playerList: Array.from(this.players.values()).map(p => ({ nickname: p.nickname, id: p.id }))
        }
      })
      
      console.log(`🎮 Player ${player.nickname} joined ongoing practice match (${this.players.size} total players)`)
    } else {
  ...
  setSpectatorCamera(socketId, cameraData) {
    ...
    if (mode === 'player_follow' && followingPlayer) {
      // Validate that the player exists and is alive (real players only)
      const targetPlayer = [...this.players.values()].find(p => p.id === followingPlayer)
  ...
  // Removed bot simulation logic - pure multiplayer only

  // Get leaderboard for spectators (real players only)
...
export class TurfLootGameServer {
  ...
  // Removed persistent bot logic - this is now pure multiplayer
  startPersistentBots(room) {
    console.log('🎮 Practice room initialized for pure multiplayer - no bots')
  }
