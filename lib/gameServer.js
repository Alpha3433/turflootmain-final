...
export class TurfLootGameServer {
  ...
  getPersistentServers() {
    ...
    console.log(`🔍 Debug: persistentServers Map has ${this.persistentServers.size} entries`)
    console.log('🔍 Debug: Server IDs:', Array.from(this.persistentServers.keys()))
    
    for (const [serverId, serverInfo] of this.persistentServers) {
      console.log(`🔍 Debug: Processing server ${serverId} with info:`, serverInfo)
    ...
    console.log(`🔍 Debug: Returning ${servers.length} servers`)
    return servers
  }
