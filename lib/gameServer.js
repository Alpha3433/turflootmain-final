  ...
  verifyToken(token) {
    try {
      ...
      if (tokenHeader?.alg === 'ES256') {
        // This is a Privy token with ECDSA signature - we can't verify it without Privy's public key
        // For now, just decode it without verification for development
        console.log('⚠️ ES256 token detected (Privy token) - decoding without verification for development')
        try {
        ...
        try {
          ...
          console.error('❌ HS256 token verification failed:', verifyError.message)
          return null
        }
      }
      ...
      if (decoded) {
        ...
        const userId = decoded.userId || decoded.privyId || decoded.sub
