...
class LobbyManager {
  ...
  async allocateMatch(lobby) {
    try {
      ...
      // Derive proper WebSocket URL from environment to avoid Mixed Content errors
      const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'
      const url = new URL(baseUrl)
      
      // Use wss:// for HTTPS sites, ws:// for HTTP sites to prevent Mixed Content security errors
      ...
      console.log(`🔗 Generated secure WebSocket endpoint: ${serverEndpoint}`)
      console.log(`🔒 Protocol derived from base URL: ${baseUrl} -> ${wsProtocol}//`)
