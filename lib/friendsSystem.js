...
export class FriendsSystem {
  ...
  static async searchUsers(query, currentUserId, onlineOnly = false, showAll = false) {
    const db = await getDb()

    // If showAll is false and no query, return empty
    if (!showAll && (!query || query.length < 2)) {
      return []
    }
    ...
    // Add username filter only if we have a query
