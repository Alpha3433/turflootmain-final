...
export const buildEntryFeeTransaction = async ({
  ...
  const transferLamports = fees.entryLamports
  ...
  const rawSerialized = transaction.serialize({
    requireAllSignatures: false,
    verifySignatures: false
  })

  const serializedBytes =
    rawSerialized instanceof Uint8Array ? rawSerialized : new Uint8Array(rawSerialized)
