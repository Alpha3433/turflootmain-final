...
const sendTransactionWithPrivy = async ({
  ...
  try {
    ...
    transactionBytes = serialised instanceof Uint8Array ? serialised : new Uint8Array(serialised)
  } catch (error) {
    console.warn('⚠️ Failed to serialize transaction for Privy request payload:', error)
    throw new Error('Unable to serialize Solana transaction before calling Privy signAndSendTransaction.')
  }

  const request = {
    chain,
    transaction: transactionBytes,
    serializedTransaction: transactionBytes
  }
  ...
  return {
    ...
    rawTransaction: transactionBytes || toUint8Array(response?.rawTransaction),
