...
const DEFAULT_AUTO_SWEEP_DESTINATION = 'F7zDew151bya8KatZiHF6EXDBi8DVNJvrLE619vwypvG'
const DEFAULT_SOLANA_NETWORK = 'mainnet-beta'
const DEFAULT_AUTO_SWEEP_MINIMUM_LAMPORTS = Math.max(
  DEFAULT_NETWORK_FEE_LAMPORTS_BUFFER * 2,
  Number(
...
let hasWarnedAboutNonMainnetNetwork = false

const warnNonMainnetNetwork = (network) => {
  if (hasWarnedAboutNonMainnetNetwork) {
    return
  }

  hasWarnedAboutNonMainnetNetwork = true
  console.warn(
    `⚠️ Unsupported Solana network "${network}" detected. Paid room flows require mainnet; defaulting to ${DEFAULT_SOLANA_NETWORK}.`
  )
}

const sanitiseSolanaNetwork = (network) => {
  ...
  if (candidate.startsWith('solana:')) {
    return sanitiseSolanaNetwork(candidate.split(':').slice(1).join(':'))
  }
  ...
  warnNonMainnetNetwork(candidate)
  return DEFAULT_SOLANA_NETWORK
}
...
export const buildSolanaRpcEndpointList = ({
  ...
  const endpoints = []
  const effectiveNetwork = sanitiseSolanaNetwork(network)
  ...
  try {
    endpoints.push(clusterApiUrl(effectiveNetwork))
  } catch (error) {
...
const deriveSolanaChainIdentifier = (chain) => {
  ...
  if (normalised.startsWith('solana:')) {
    return deriveSolanaChainIdentifier(normalised.split(':').slice(1).join(':'))
  }
  ...
  warnNonMainnetNetwork(normalised)
  return 'solana:mainnet'
}
...
const normaliseAddressString = (value) => {
  if (!value) {
    return null
  }
...
export const deductPaidRoomFee = async ({
  ...
  const modalUiOptions = {
    showWalletUIs: true,
    description: `Approve ${formatUsd(totalCostUsd)} (${formatSol(totalCostSol)}) arena entry plus ~${formatUsd(networkFeeUsdEstimate)} network fees.`,
    buttonText: 'Confirm entry',
    transactionInfo: {
      title: 'TurfLoot Arena Entry',
      action: `Join arena (${formatUsd(effectiveCosts.entryFeeUsd)} entry)`,
      contractInfo: {
...
export const AUTO_SWEEP_MINIMUM_LAMPORTS = DEFAULT_AUTO_SWEEP_MINIMUM_LAMPORTS

export async function autoSweepEmbeddedWallet({
  solanaWallet,
  privyUser,
  signAndSendTransactionFn,
  rpcEndpoints,
  ...
  sourceAddress,
  destinationAddress,
  minimumLamports = DEFAULT_AUTO_SWEEP_MINIMUM_LAMPORTS,
  memo,
  logger = console
}) {
  const log = logger || console
  ...
  if (typeof signAndSendTransactionFn !== 'function') {
    throw new Error('No Privy signAndSendTransaction function available for auto sweep.')
  }

  const walletAddress =
    normaliseAddressString(sourceAddress) ||
    normaliseAddressString(solanaWallet?.address) ||
    normaliseAddressString(solanaWallet?.publicKey) ||
    normaliseAddressString(privyUser?.wallet?.address)

  if (!walletAddress) {
    throw new Error('No Solana wallet address available for auto sweep.')
  }

  const destination =
    normaliseAddressString(destinationAddress) ||
    normaliseAddressString(process.env.NEXT_PUBLIC_SITE_FEE_WALLET) ||
    normaliseAddressString(process.env.SITE_FEE_WALLET) ||
    DEFAULT_AUTO_SWEEP_DESTINATION

  if (!destination) {
    throw new Error('No destination wallet configured for auto sweep.')
  }
  ...
  const balanceLamports = await connection.getBalance(fromPublicKey, 'confirmed')
  const configuredMinimumLamports = Number.isFinite(minimumLamports) ? Math.max(0, Math.floor(minimumLamports)) : 0
  const minTransferLamports = Math.max(
    configuredMinimumLamports,
    DEFAULT_NETWORK_FEE_LAMPORTS_BUFFER * 2
  )

  log.log?.('🔎 Auto sweep balance check:', {
    walletAddress,
    destination,
    balanceLamports,
    minTransferLamports,
  ...
  if (balanceLamports <= minTransferLamports) {
    log.log?.('ℹ️ Auto sweep skipped: balance below threshold.', {
      walletAddress,
      destination,
      balanceLamports,
      minTransferLamports,
    ...
    return {
      ...
      destination,
      balanceLamports,
      thresholdLamports: minTransferLamports,
      rpcEndpoint: sanitisedEndpoint
    }
  }
  ...
  const buildTransaction = (lamports) => {
    const tx = new Transaction()
    ...
    if (memoPayload) {
      const memoData =
        typeof Buffer !== 'undefined'
          ? Buffer.from(memoPayload, 'utf8')
          : new TextEncoder().encode(memoPayload)
  ...
  let transferLamports = Math.max(0, balanceLamports - DEFAULT_NETWORK_FEE_LAMPORTS_BUFFER)
  if (transferLamports < minTransferLamports) {
    log.log?.('ℹ️ Auto sweep skipped: transfer amount below minimum threshold.', {
      walletAddress,
      destination,
      transferLamports,
      minTransferLamports,
  ...
  let transaction = buildTransaction(transferLamports)
  let estimatedNetworkFeeLamports = DEFAULT_NETWORK_FEE_LAMPORTS_BUFFER
  ...
  try {
    ...
    if (feeResult && typeof feeResult.value === 'number' && feeResult.value > 0) {
      ...
      const adjustedLamports = Math.max(0, balanceLamports - feeResult.value)

      if (adjustedLamports < minTransferLamports) {
        log.log?.('ℹ️ Auto sweep skipped after fee estimation: amount below threshold.', {
          walletAddress,
          destination,
          adjustedLamports,
          minTransferLamports,
      ...
      if (adjustedLamports !== transferLamports) {
        transferLamports = adjustedLamports
        transaction = buildTransaction(transferLamports)
      }
    }
  } catch (error) {
    log.warn?.('⚠️ Unable to estimate network fee for auto sweep, using default buffer.', error)
  }

  const chainIdentifier = deriveSolanaChainIdentifier(solanaChain)
  const sweepSol = transferLamports / LAMPORTS_PER_SOL
  const modalUiOptions = {
    showWalletUIs: true,
    description: `Transfer ${formatSol(sweepSol)} to TurfLoot vault`,
    buttonText: 'Approve transfer',
    transactionInfo: {
      title: 'TurfLoot Auto Sweep',
      action: `Secure ${formatSol(sweepSol)} in TurfLoot treasury`,
      contractInfo: {
        name: 'TurfLoot Treasury',
        url: 'https://turfloot.gg'
      }
    },
    successHeader: 'Funds secured',
    successDescription: 'Your embedded wallet balance has been secured in the TurfLoot treasury.'
  }
  const preflightOptions = {
    preflightCommitment: 'confirmed',
    commitment: 'confirmed',
  ...
  log.log?.('🏦 Initiating auto sweep for embedded wallet.', {
    walletAddress,
    destination,
    sweepLamports: transferLamports,
    sweepSol,
    balanceLamports,
    rpcEndpoint: sanitisedEndpoint,
    chain: chainIdentifier
  ...
  try {
    ...
    if (!signature) {
      throw new Error('Privy wallet did not return a signature for the auto sweep transaction.')
    }
    ...
    log.error?.('❌ Auto sweep transaction failed.', error)
    throw error
  }

  log.log?.('✅ Auto sweep confirmed on Solana.', {
    walletAddress,
    destination,
    signature,
    rpcEndpoint: sanitisedEndpoint,
