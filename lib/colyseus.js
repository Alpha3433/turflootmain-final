...
class TurfLootColyseusClient {
  ...
  async joinArena({ privyUserId, playerName = null, specificRoomId = null }) {
    try {
      ...
      // ALWAYS join the persistent 24/7 room - ignore specificRoomId
      // This ensures all players end up in the same room instance
      console.log('🎯 Joining persistent 24/7 TurfLoot arena...')
      this.room = await this.client.joinOrCreate("arena", {
        privyUserId: privyUserId,
        playerName: playerName || `Player_${Math.random().toString(36).substring(7)}`,
        roomName: "turfloot-persistent-arena-24-7" // Fixed persistent room name
      })
      console.log('✅ Connected to persistent 24/7 arena room')
