...
class TurfLootColyseusClient {
  ...
  async joinArena({ privyUserId, playerName = null, specificRoomId = null }) {
    try {
      ...
      // Try to join existing rooms first, then create if none exist
      console.log('🔍 Looking for existing arena rooms...')
      
      try {
        // First, try to join any existing arena room
        const availableRooms = await this.client.getAvailableRooms("arena")
        console.log('📊 Available arena rooms:', availableRooms.length)
        
        if (availableRooms.length > 0) {
          // Join the first available room
          const roomToJoin = availableRooms[0]
          console.log('🎯 Joining existing room:', roomToJoin.roomId)
          
          this.room = await this.client.joinById(roomToJoin.roomId, {
            privyUserId: privyUserId,
            playerName: playerName || `Player_${Math.random().toString(36).substring(7)}`
          })
          console.log('✅ Successfully joined existing room:', roomToJoin.roomId)
        } else {
          // No rooms exist, create a new one
          console.log('🏗️ No existing rooms, creating new arena...')
          this.room = await this.client.create("arena", {
            privyUserId: privyUserId,
            playerName: playerName || `Player_${Math.random().toString(36).substring(7)}`,
            roomName: "global-turfloot-arena"
          })
          console.log('✅ Created new arena room:', this.room.roomId)
        }
      } catch (error) {
        console.warn('⚠️ Failed to join existing room, falling back to joinOrCreate:', error.message)
        
        // Fallback to joinOrCreate with very specific matching criteria
        this.room = await this.client.joinOrCreate("arena", {
          privyUserId: privyUserId,
          playerName: playerName || `Player_${Math.random().toString(36).substring(7)}`,
          roomName: "global-turfloot-arena",
          persistent: true // Additional matching criteria
        })
