...
class TurfLootColyseusClient {
  ...
  async joinArena({ privyUserId, playerName = null, specificRoomId = null }) {
    try {
      ...
      // Use the simplest approach - just use joinOrCreate with minimal consistent options
      console.log('🎯 Joining global TurfLoot arena...')
      
      this.room = await this.client.joinOrCreate("arena", {
        // Use minimal options to maximize room matching
      })
    ...
    this.room.onStateChange.once((state) => {
      if (state && state.players) {
        ...
        state.players.onAdd = (player, sessionId) => {
          console.log(`👋 Player joined: ${player?.name || player?.playerName || 'Unknown Player'} (${sessionId})`);
        };

        // Handle player removals
        state.players.onRemove = (player, sessionId) => {
          console.log(`👋 Player left: ${player?.name || player?.playerName || 'Player'} (${sessionId})`);
        };
