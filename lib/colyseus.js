...
class TurfLootColyseusClient {
  ...
  setupEventListeners() {
    ...
    this.room.onStateChange((state) => {
      if (!state) {
        console.log('📊 Game state updated: state is null/undefined')
        return
      }
      
      console.log('📊 Game state updated:', {
        players: state.players ? state.players.size || Object.keys(state.players).length || 0 : 0,
        coins: state.coins ? state.coins.size || Object.keys(state.coins).length || 0 : 0,
        viruses: state.viruses ? state.viruses.size || Object.keys(state.viruses).length || 0 : 0,
        timestamp: state.timestamp || Date.now()
      })
    })

    // Handle player additions (with null check)
