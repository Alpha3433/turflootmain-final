...
class TurfLootColyseusClient {
  ...
  async joinArena({ privyUserId, playerName = null, specificRoomId = null }) {
    try {
      ...
      console.log('👤 Player info:', { privyUserId, playerName, specificRoomId })
      
      // Create Colyseus client
      this.client = new Client(this.endpoint)
      
      if (specificRoomId) {
        // Join specific existing room by ID
        console.log('🎯 Joining specific room:', specificRoomId)
        this.room = await this.client.join(specificRoomId, {
          privyUserId: privyUserId,
          playerName: playerName || `Player_${Math.random().toString(36).substring(7)}`
        })
        console.log('✅ Successfully joined existing room:', specificRoomId)
      } else {
        // Join or create the global arena room
        console.log('🎯 Joining or creating global TurfLoot arena...')
        this.room = await this.client.joinOrCreate("arena", {
          privyUserId: privyUserId,
          playerName: playerName || `Player_${Math.random().toString(36).substring(7)}`,
          roomName: "global-turfloot-arena" // Force all players into same room
        })
        console.log('✅ Connected to arena room via joinOrCreate')
      }
