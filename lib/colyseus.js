...
class TurfLootColyseusClient {
  ...
  async joinArena({ privyUserId, playerName = null, specificRoomId = null }) {
    try {
      ...
      // Use a much simpler approach - minimal options to ensure room matching
      ...
      this.room = await this.client.joinOrCreate("arena", {
        // Use ONLY essential matching criteria
        roomName: "global-turfloot-arena"
        // Don't include dynamic data that might prevent room matching
      })
      ...
      console.log(`🔧 Room options used: roomName=global-turfloot-arena ONLY`)
      
      // Send player data after connection via room messages
      if (playerName || privyUserId) {
        try {
          this.room.send("updatePlayerInfo", {
            playerName: playerName || 'Anonymous Player',
            privyUserId: privyUserId
          })
          console.log('📤 Sent player info to room:', { playerName, privyUserId })
        } catch (error) {
          console.warn('⚠️ Could not send player info to room:', error)
        }
      }
