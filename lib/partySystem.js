...
export class PartySystem {
  ...
  static async createParty(ownerId, ownerUsername, partyName = null) {
    ...
    console.log(`🔍 createParty: Checking for existing membership for userId=${ownerId}`)
    const existingMembership = await db.collection('party_members').findOne({
      userId: ownerId,
      status: 'joined'
    }, { sort: { joinedAt: -1 } })

    console.log(`🔍 createParty: Existing membership found:`, existingMembership)

    if (existingMembership) {
      // Get the party details for better error message
      console.log(`🔍 createParty: Checking if party ${existingMembership.partyId} is still active`)
      const existingParty = await db.collection('parties').findOne({
        id: existingMembership.partyId,
        status: { $in: ['active', 'waiting'] }
      })
      
      console.log(`🔍 createParty: Existing party found:`, existingParty)
      
      if (existingParty) {
        console.log(`🔍 createParty: User already has active party, throwing error`)
        throw new Error(`You already have an active party: "${existingParty.name}". Leave your current party first.`)
      } else {
        console.log(`🔍 createParty: Existing party is not active, cleaning up stale membership`)
  ...
  static async getUserParty(userId) {
    ...
    // Get the most recent active membership (sort by joinedAt descending)
    const membership = await db.collection('party_members').findOne({
      userId,
      status: 'joined'
    }, { sort: { joinedAt: -1 } })

    console.log(`🔍 getUserParty: Most recent active membership found:`, membership)
    ...
    if (!party) {
      ...
      // Clean up stale membership record
      await db.collection('party_members').updateOne(
        { _id: membership._id },
        { $set: { status: 'stale', updatedAt: new Date().toISOString() } }
      )
      console.log(`🔍 getUserParty: Marked stale membership as 'stale'`)
      
      // Try to find another active membership
      const nextMembership = await db.collection('party_members').findOne({
        userId,
        status: 'joined',
      ...
      if (nextMembership) {
        console.log(`🔍 getUserParty: Found another membership, checking party:`, nextMembership)
        const nextParty = await db.collection('parties').findOne({
          id: nextMembership.partyId,
          status: { $in: ['waiting', 'active'] }
        })
        
        if (nextParty) {
          console.log(`🔍 getUserParty: Found valid party for second membership`)
          // Continue with this party and membership
