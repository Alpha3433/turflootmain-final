...
class TurfLootHathoraClient {
  ...
  async initialize() {
    ...
    try {
      ...
      this.hathoraCloud = new HathoraCloud({
        appId: this.appId,
      })
  ...
  async getConnectionInfo(roomId) {
    try {
      if (!this.roomClient) {
        throw new Error('Room client not initialized')
      }
      
      // Use RoomV2Api to get connection info
      const response = await this.roomClient.getConnectionInfo(this.appId, roomId)
