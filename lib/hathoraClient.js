  ...
  async createPaidRoom(stakeAmount, userId = null, region = null) {
    ...
    try {
      ...
      try {
        ...
        // Use createRoom instead of createPublicLobby
        ...
        console.log(`📤 Creating room with config:`, createRoomConfig)
        roomId = await this.client.createRoom(createRoomConfig)
        
        console.log(`📊 Raw createRoom result:`, roomId)
        ...
        if (roomId) {
          ...
          console.error(`❌ createRoom returned invalid result: ${roomId}`)
          throw new Error(`Hathora createRoom returned invalid result: ${roomId}`)
        }
      } catch (regionError) {
        ...
        try {
          // Fallback - create room without region specification
          const fallbackConfig = {
            visibility: 'public',
            roomConfig: {
              gameMode: 'cash-game',
          ...
          roomId = await this.client.createRoom(fallbackConfig)
          
          console.log(`📊 Fallback createRoom result:`, roomId)
          console.log(`📊 Fallback result type: ${typeof roomId}`)
          
          if (roomId) {
            console.log(`✅ Created Hathora room (no region specified): ${roomId}`)
            ...
            console.error(`❌ Fallback createRoom also returned invalid result: ${roomId}`)
            throw new Error(`Hathora createRoom fallback returned invalid result: ${roomId}`)
          }
        } catch (fallbackError) {
          console.error(`❌ Failed to create room even without region:`, fallbackError.message)
          throw fallbackError
