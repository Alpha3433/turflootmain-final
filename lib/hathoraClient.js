  ...
  async createOrJoinRoom(userId = null, gameMode = 'practice', stakeAmount = 0) {
    ...
    try {
      ...
      try {
        ...
        const fallbackResult = await this.client.createRoom(fallbackConfig)
        console.log(`📊 Fallback createRoom result:`, fallbackResult)
        
        if (fallbackResult && typeof fallbackResult === 'string' && fallbackResult.length > 0) {
          roomId = fallbackResult
          console.log(`✅ Created Hathora room (no region): ${roomId}`)
          ...
          throw new Error(`Fallback createRoom also returned invalid result: ${fallbackResult}`)
        }
      } catch (fallbackError) {
