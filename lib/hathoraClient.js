...
class TurfLootHathoraClient {
  ...
  async createOrJoinRoom(userId = null, gameMode = 'practice', stakeAmount = 0) {
    ...
    try {
      // Create room with new SDK API - createRoom instead of createPublicLobby
      console.log(`🎯 Attempting to create room with region: ${roomConfig.region}`)
      ...
      const roomResult = await this.client.createRoom(createRoomConfig)
