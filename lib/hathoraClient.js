...
class TurfLootHathoraClient {
  ...
  async connectToGame(gameConfig = {}) {
    ...
    // For client-side, we don't need authentication tokens for WebSocket connections
    // The server API handles authentication when creating rooms
    console.log('🌍 Creating direct WebSocket connection to Hathora server')
    
    if (finalRoomId) {
      // Use direct connection with room ID
      socketUrl = `wss://${serverInfo.host}:${serverInfo.port}?roomId=${finalRoomId}`
      console.log('🌍 Connecting to Hathora server with room ID parameter')
    } else {
