  ...
  async createOrJoinRoom(userId = null, gameMode = 'practice', stakeAmount = 0) {
    ...
    try {
      ...
      try {
        ...
        const fallbackResult = await this.client.createPublicLobby(token, fallbackOptions)
        console.log(`📊 Fallback createPublicLobby result:`, fallbackResult)
        ...
        if (fallbackResult && typeof fallbackResult === 'string' && fallbackResult.length > 0) {
          ...
          throw new Error(`Fallback createPublicLobby also returned invalid result: ${fallbackResult}`)
        }
      } catch (fallbackError) {
