...
import { RoomV2Api } from '@hathora/cloud-sdk-typescript'

class TurfLootHathoraClient {
  constructor() {
    this.roomClient = null
    this.appId = process.env.NEXT_PUBLIC_HATHORA_APP_ID
    this.developerToken = process.env.HATHORA_DEVELOPER_TOKEN
  ...
  async initialize() {
    ...
    try {
      ...
      console.log('🌍 Hathora RoomV2Api client initialized successfully')
  ...
  async getConnectionInfo(roomId) {
    try {
      if (!this.hathoraCloud) {
        throw new Error('Hathora client not initialized')
      }
      
      // Use HathoraCloud roomsV2.getConnectionInfo
      const response = await this.hathoraCloud.roomsV2.getConnectionInfo(roomId, this.appId)
      return response
    } catch (error) {
      console.warn(`⚠️ Failed to get connection info for room ${roomId}:`, error.message)
  ...
  async createOrJoinRoom(userId = null, gameMode = 'practice', stakeAmount = 0) {
    ...
    try {
      // Use RoomV2Api.createRoom with proper authentication
      const mappedRegion = this.mapServerRegionToHathora(roomConfig.region)
      console.log(`🎯 Creating room in region: ${mappedRegion}`)
      
      let roomResponse
      try {
        // Create room using RoomV2Api.createRoom method
        roomResponse = await this.roomClient.createRoom(
          this.appId,
          { region: mappedRegion },
          undefined, // roomId (let Hathora generate)
          {
            headers: {
        ...
        console.log(`✅ Created Hathora room:`, roomResponse)
      } catch (regionError) {
        ...
        console.log(`✅ Created Hathora room (no region):`, roomResponse)
      }

      if (!roomResponse || !roomResponse.roomId) {
        throw new Error(`Invalid room response: ${roomResponse}`)
      }
  ...
  async createPaidRoom(stakeAmount, userId = null, region = null) {
    ...
    try {
      console.log('🔐 Creating room using RoomV2Api...')
      
      // Create room with correct API
      console.log(`🌍 Creating Hathora room in region: ${roomRegion}`)
      
      let roomResponse
      try {
        // Use RoomV2Api.createRoom (correct API)
        roomResponse = await this.roomClient.createRoom(
          this.appId,
          { region: roomRegion },
        ...
        console.log(`✅ Created Hathora room with region:`, roomResponse)
