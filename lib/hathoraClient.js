...
class TurfLootHathoraClient {
  ...
  async initialize() {
    if (!this.isHathoraEnabled) {
      console.log('🔧 Hathora not configured - using local server')
      return false
    }
  ...
  async createOrJoinRoom(userId = null, gameMode = 'practice') {
    if (!this.client) return 'local-room'

    try {
      // For practice mode, try to join existing practice room first
      if (gameMode === 'practice') {
        ...
        if (practiceRooms.length > 0) {
          console.log('🎮 Joining existing practice room')
  ...
  getPreferredRegion() {
    ...
    if (timezone.includes('America')) {
      return timezone.includes('Los_Angeles') ? 'seattle' : 'washington-dc'
    } else if (timezone.includes('Europe')) {
      return 'london'
    } else if (timezone.includes('Asia')) {
      if (timezone.includes('Tokyo')) return 'tokyo'
