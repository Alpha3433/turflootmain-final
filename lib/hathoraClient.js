...
class TurfLootHathoraClient {
  ...
  async connectToGame(gameConfig = {}) {
    ...
    if (requestedRoomId === 'global-practice-bots' || requestedRoomId?.includes('global')) {
      ...
      if (token) {
        // Use authenticated WebSocket URL with query parameters format
        socketUrl = `wss://${connectionInfo.host}:${connectionInfo.port}/ws?token=${token}&roomId=${roomId}`
        console.log('🌍 Connecting to authenticated Hathora server process with query params')
      } else {
        // Fallback to basic WebSocket path
        socketUrl = `wss://${connectionInfo.host}:${connectionInfo.port}/ws`
        console.log('🌍 Connecting to basic Hathora server process (no auth)')
      }
    ...
    if (token && finalRoomId) {
      // Use authenticated WebSocket URL with room ID and token
      socketUrl = `wss://${serverInfo.host}:${serverInfo.port}/${finalRoomId}?token=${token}`
      console.log('🌍 Connecting to authenticated Hathora server process')
    } else {
      // Fallback to basic WebSocket path
      socketUrl = `wss://${serverInfo.host}:${serverInfo.port}/ws`
      console.log('🌍 Connecting to basic Hathora server process (no auth)')
    }
