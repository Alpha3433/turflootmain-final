...
class TurfLootHathoraClient {
  ...
  async createOrJoinRoom(userId = null, gameMode = 'practice', stakeAmount = 0) {
    ...
    try {
      ...
      const regionResult = await this.client.createPublicLobby(token, { region: roomConfig.region })
      console.log(`📊 Raw createPublicLobby result:`, regionResult)
      console.log(`📊 Result type:`, typeof regionResult)
      
      if (regionResult && typeof regionResult === 'string' && regionResult.length > 0) {
        roomId = regionResult
        console.log(`✅ Created Hathora room with region: ${roomId} in ${roomConfig.region}`)
      } else {
        console.error(`❌ createPublicLobby returned invalid result:`, regionResult)
        throw new Error(`Hathora createPublicLobby returned invalid result: ${regionResult}`)
      }
    } catch (regionError) {
      ...
      try {
        ...
        const fallbackResult = await this.client.createPublicLobby(token)
  ...
  mapServerRegionToHathora(serverRegion) {
    ...
    const regionMap = {
      // Full region names mapped to Hathora canonical region codes (uppercase)
      'US East': 'SEATTLE',
      'US-East-1': 'SEATTLE', 
      'US West': 'LOS_ANGELES',
      'Europe': ['FRANKFURT', 'LONDON'], // EU regions
      'EU': ['FRANKFURT', 'LONDON'],
      'Europe (Frankfurt)': 'FRANKFURT',
      'Europe (London)': 'LONDON',
      'Oceania': 'SYDNEY', // Proper Oceania region
      'OCE (Sydney)': 'SYDNEY',
      'Asia': 'SINGAPORE',
      ...
      'US': 'SEATTLE',
      'EU': ['FRANKFURT', 'LONDON'],
      'OCE': 'SYDNEY', // Use proper Sydney region for Oceania
      'SEA': 'SINGAPORE',
      
      // Specific region IDs from server browser mapped to Hathora canonical region codes
