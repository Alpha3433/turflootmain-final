...
class TurfLootHathoraClient {
  ...
  async createOrJoinRoom(userId = null, gameMode = 'practice', stakeAmount = 0) {
    ...
    try {
      ...
      console.log(`📊 Raw createPublicLobby result:`, roomResult)
      console.log(`📊 Result type:`, typeof roomResult)
      
      if (roomResult && typeof roomResult === 'string' && roomResult.length > 0) {
        roomId = roomResult
        ...
        console.error(`❌ createPublicLobby returned invalid result:`, roomResult)
        throw new Error(`Hathora createPublicLobby returned invalid result: ${roomResult}`)
      }
    } catch (regionError) {
