...
class TurfLootHathoraClient {
  ...
  async createOrJoinRoom(userId = null, gameMode = 'practice', stakeAmount = 0) {
    ...
    try {
      ...
      console.log(`📊 Raw createRoom result:`, roomResult)
      console.log(`📊 Result type:`, typeof roomResult)
      
      if (roomResult && typeof roomResult === 'string' && roomResult.length > 0) {
        roomId = roomResult
        console.log(`✅ Created Hathora room: ${roomId} in region ${roomConfig.region}`)
        ...
        console.error(`❌ createRoom returned invalid result:`, roomResult)
        throw new Error(`Hathora createRoom returned invalid result: ${roomResult}`)
      }
    } catch (regionError) {
