...
class TurfLootHathoraClient {
  ...
  async createOrJoinRoom(userId = null, gameMode = 'practice', stakeAmount = 0) {
    ...
    let roomId
    try {
      // Try to create room with region parameter (if supported by SDK)
      roomId = await this.client.createPublicLobby(token, { region: roomConfig.region })
      console.log(`✅ Created Hathora room with region config: ${roomId} in ${roomConfig.region}`)
    } catch (regionError) {
      console.warn(`⚠️ Failed to create room with region config, trying without region:`, regionError.message)
  ...
  async createPaidRoom(stakeAmount, userId = null, region = null) {
    ...
    try {
      ...
      // Create room - try with region first, fallback to no region
      console.log(`🌍 Attempting to create Hathora lobby in region: ${roomRegion}`)
      
      let roomId
      try {
        // Most Hathora SDKs expect region as a separate parameter or in options
        roomId = await this.client.createPublicLobby(token, roomRegion)
        console.log(`✅ Created Hathora room with region parameter: ${roomId} in ${roomRegion}`)
      } catch (regionError) {
        console.log(`⚠️ Region parameter failed, trying alternative syntax:`, regionError.message)
        try {
          // Alternative syntax - region in options object
          roomId = await this.client.createPublicLobby(token, { region: roomRegion })
          console.log(`✅ Created Hathora room with region config: ${roomId} in ${roomRegion}`)
        } catch (configError) {
          console.log(`⚠️ Region config failed, creating without region:`, configError.message)
          // Final fallback - no region (will default to Washington D.C.)
          roomId = await this.client.createPublicLobby(token)
          console.log(`✅ Created Hathora room (no region): ${roomId} - may default to Washington D.C.`)
        }
      }
  ...
  mapServerRegionToHathora(serverRegion) {
    const regionMap = {
      ...
      // Specific region IDs from server browser
