...
class TurfLootHathoraClient {
  ...
  async connectToGame(gameConfig = {}) {
    ...
    // FIXED: For global-practice-bots, use a simpler approach - connect to our server but with shared room logic
    if (requestedRoomId === 'global-practice-bots') {
      console.log('🌍 Global Multiplayer detected - using shared room approach')
      
      // For now, connect to our local server but ensure room joining logic handles the shared aspect
      // This bypasses complex Hathora room creation which might be failing
      const { io } = await import('socket.io-client')
      const socketUrl = process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'
      
      console.log('🌍 Connecting to shared server for global multiplayer:', socketUrl)
      
      const socket = io(socketUrl, {
        transports: ['websocket', 'polling'],
        timeout: 15000,
        forceNew: false, // Important: Don't force new connections
      ...
      this.currentConnection = {
        ...
        serverInfo: {
          ...
          port: 3000,
          isLocal: false, // Treat as shared server
          region: 'US-East-1',
          roomId: requestedRoomId,
          isSharedGlobalRoom: true // Flag to indicate this is the shared global room
        },
        roomId: requestedRoomId
      }

      console.log('✅ Global Multiplayer connection established (shared room approach):', {
        socketUrl,
        roomId: requestedRoomId,
        sharedRoom: true
      })
