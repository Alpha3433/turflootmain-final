...
class TurfLootHathoraClient {
  ...
  // Connect to Socket.IO - CREATE ACTUAL HATHORA PROCESSES for global multiplayer
  async connectToGame(gameConfig = {}) {
    ...
    // FIXED: Create actual Hathora processes for global multiplayer
    if (requestedRoomId === 'global-practice-bots') {
      console.log('🌍 Global Multiplayer detected - creating Hathora process')
      ...
      try {
        ...
        console.error('❌ Hathora process creation failed:', error)
        console.log('🔄 Falling back to local server for global multiplayer')
        ...
        this.currentConnection = {
          ...
          serverInfo: {
            ...
            isEnhancedGlobalServer: true,
            note: 'Local server fallback due to Hathora failure'
          },
          roomId: requestedRoomId
        }

        console.log('✅ Fallback Global Multiplayer connection established:', {
          socketUrl,
          roomId: requestedRoomId,
          serverType: 'Local Fallback Server'
        })
