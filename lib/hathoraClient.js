...
class TurfLootHathoraClient {
  ...
  async createOrJoinRoom(userId = null, gameMode = 'practice', stakeAmount = 0) {
    ...
    try {
      // Create lobby with updated SDK API - using createPublicLobby with proper options
      ...
      console.log(`📤 Creating lobby with options:`, lobbyOptions)
      const roomResult = await this.client.createPublicLobby(token, lobbyOptions)
