...
class TurfLootHathoraClient {
  ...
  async createOrJoinRoom(userId = null, gameMode = 'practice', stakeAmount = 0) {
    ...
    // Use createRoom to create actual server process with proper configuration
    ...
    try {
      // Create lobby with new SDK API - createLobby instead of createPublicLobby
      console.log(`🎯 Attempting to create lobby with region: ${roomConfig.region}`)
      
      const createLobbyConfig = {
        visibility: 'public', // or 'private' depending on requirements
      ...
      console.log(`📤 Creating lobby with config:`, createLobbyConfig)
      const roomResult = await this.client.lobbiesV3.createLobby(token, createLobbyConfig)
      console.log(`📊 Raw createLobby result:`, roomResult)
      console.log(`📊 Result type:`, typeof roomResult)
      
      if (roomResult && roomResult.roomId && typeof roomResult.roomId === 'string' && roomResult.roomId.length > 0) {
        roomId = roomResult.roomId
        console.log(`✅ Created Hathora lobby: ${roomId} in region ${roomConfig.region}`)
        
        // Immediately fetch connection details with getRoomInfo
        console.log(`📡 Fetching room connection info for: ${roomId}`)
        try {
          connectionInfo = await this.client.getRoomInfo(roomId)
          console.log(`📊 Room connection info:`, connectionInfo)
          
          if (connectionInfo && connectionInfo.host && connectionInfo.port) {
            console.log(`🌐 Real connection details: ${connectionInfo.host}:${connectionInfo.port}`)
          } else {
            console.warn(`⚠️ getRoomInfo returned incomplete connection info:`, connectionInfo)
          }
        } catch (roomInfoError) {
          console.warn(`⚠️ Failed to get room connection info:`, roomInfoError.message)
          // Continue without connection info - will use fallback
        }
        
      } else {
        console.error(`❌ createLobby returned invalid result:`, roomResult)
        throw new Error(`Hathora createLobby returned invalid result: ${roomResult}`)
      }
    } catch (regionError) {
      console.warn(`⚠️ Region-specific room creation failed:`, regionError.message)
      console.log(`🔄 Trying without region specification...`)
      ...
      try {
        ...
        const fallbackResult = await this.client.lobbiesV3.createLobby(token, fallbackConfig)
        console.log(`📊 Fallback createLobby result:`, fallbackResult)
        
        if (fallbackResult && fallbackResult.roomId && typeof fallbackResult.roomId === 'string' && fallbackResult.roomId.length > 0) {
          roomId = fallbackResult.roomId
          console.log(`✅ Created Hathora lobby (no region): ${roomId}`)
          
          // Try to get connection info for fallback room too
          try {
            connectionInfo = await this.client.getRoomInfo(roomId)
            console.log(`📊 Fallback room connection info:`, connectionInfo)
          } catch (fallbackRoomInfoError) {
            console.warn(`⚠️ Failed to get fallback room connection info:`, fallbackRoomInfoError.message)
          }
          
        } else {
          throw new Error(`Fallback createLobby also returned invalid result: ${fallbackResult}`)
        }
      } catch (fallbackError) {
    ...
    if (!roomId || typeof roomId !== 'string' || roomId.length === 0) {
      throw new Error(`Failed to create Hathora room - invalid room ID: ${roomId}`)
    }
    
    console.log(`🆕 Created Hathora server process: ${roomId} in region ${roomConfig.region || 'default'}`)
    console.log(`💰 Room type: ${gameMode}, Entry fee: $${stakeAmount}`)
    
    // Return both room ID and real connection info
    const result = {
      roomId: roomId,
      region: roomConfig.region,
      entryFee: stakeAmount,
      maxPlayers: roomConfig.maxPlayers || 8,
      gameMode: gameMode,
      connectionInfo: connectionInfo || null // Real connection info or null
    }
    
    console.log(`📊 Final createPaidRoom result:`, result)
    return result
  }
  ...
  async createPaidRoom(stakeAmount, userId = null, region = null) {
    ...
    try {
      ...
      let roomId
      let connectionInfo = null // Store real connection info
      ...
      try {
        ...
        // Use createLobby to create actual server process with proper configuration
        const createLobbyConfig = {
          visibility: 'public',
          region: roomRegion,
          roomConfig: JSON.stringify({
        ...
        console.log(`📤 Creating lobby with config:`, createLobbyConfig)
        roomId = await this.client.lobbiesV3.createLobby(token, createLobbyConfig)
        
        console.log(`📊 Raw createLobby result:`, roomId)
        ...
        if (roomId && roomId.roomId) {
          ...
          // Immediately fetch real connection details
          console.log(`📡 Fetching room connection info for: ${roomId}`)
          try {
            const realConnectionInfo = await this.client.getRoomInfo(roomId)
            console.log(`📊 Real room connection info:`, realConnectionInfo)
            
            if (realConnectionInfo && realConnectionInfo.host && realConnectionInfo.port) {
              console.log(`🌐 Real connection details: ${realConnectionInfo.host}:${realConnectionInfo.port}`)
              connectionInfo = realConnectionInfo
            } else {
              console.warn(`⚠️ getRoomInfo returned incomplete connection info:`, realConnectionInfo)
            }
          } catch (roomInfoError) {
            console.warn(`⚠️ Failed to get room connection info:`, roomInfoError.message)
          }
          ...
          console.error(`❌ createLobby returned invalid result: ${roomId}`)
          throw new Error(`Hathora createLobby returned invalid result: ${roomId}`)
        }
      } catch (regionError) {
        ...
        try {
          // Fallback - create lobby without region specification
          const fallbackConfig = {
            visibility: 'public',
            roomConfig: JSON.stringify({
              gameMode: 'cash-game',
          ...
          roomId = await this.client.lobbiesV3.createLobby(token, fallbackConfig)
          
          console.log(`📊 Fallback createLobby result:`, roomId)
          console.log(`📊 Fallback result type: ${typeof roomId}`)
          
          if (roomId && roomId.roomId) {
            roomId = roomId.roomId
            console.log(`✅ Created Hathora lobby (no region specified): ${roomId}`)
            
            // Try to get connection info for fallback room too
            try {
              const fallbackConnectionInfo = await this.client.getRoomInfo(roomId)
              console.log(`📊 Fallback room connection info:`, fallbackConnectionInfo)
              if (fallbackConnectionInfo && fallbackConnectionInfo.host && fallbackConnectionInfo.port) {
            ...
            console.error(`❌ Fallback createLobby also returned invalid result: ${roomId}`)
            throw new Error(`Hathora createLobby fallback returned invalid result: ${roomId}`)
          }
        } catch (fallbackError) {
          console.error(`❌ Failed to create room even without region:`, fallbackError.message)
          throw fallbackError
      ...
      // Return result with real connection info
      const result = {
        roomId: roomId,
        config: roomConfig,
        region: roomRegion, 
