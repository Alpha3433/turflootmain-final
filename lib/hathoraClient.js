...
class TurfLootHathoraClient {
  ...
  async connectToGame(gameConfig = {}) {
    ...
    if (requestedRoomId === 'global-practice-bots' || requestedRoomId?.includes('global')) {
      ...
      // Create actual room via server API instead of direct SDK calls
      const roomConfig = {
        gameMode: 'practice',
        region: this.getPreferredRegion(),
        maxPlayers: 50,
      ...
      // Use server API to create room instead of direct client calls
      const roomResponse = await this.createRoomServerSide(
        roomConfig.gameMode, 
        roomConfig.region, 
        roomConfig.maxPlayers, 
        roomConfig.stakeAmount
      )
      ...
      console.log('🆕 Created Hathora room process for global multiplayer:', roomResponse.roomId)
      
      // Use connection info from server API response
      const connectionInfo = {
        host: roomResponse.host || 'hathora.dev',
        port: roomResponse.port || 443,
        region: roomResponse.region || this.getPreferredRegion(),
      ...
      // Get token from room response and prepare WebSocket parameters
      const hathoraToken = roomResponse.token || roomResponse.connectionToken
      const hathoraRoomId = roomResponse.roomId
      
      if (hathoraToken && hathoraRoomId) {
        // Use direct connection with properly defined query parameters
        socketUrl = `wss://${connectionInfo.host}:${connectionInfo.port}?token=${encodeURIComponent(hathoraToken)}&roomId=${encodeURIComponent(hathoraRoomId)}`
        console.log('🌍 Connecting to Hathora server with token and roomId parameters')
      } else {
        // Fallback to direct connection without authentication if parameters are missing
        socketUrl = `wss://${connectionInfo.host}:${connectionInfo.port}`
        console.log('🌍 Connecting to Hathora server with direct connection (missing auth parameters)')
        console.warn('⚠️ Missing token or roomId - WebSocket handshake may fail')
      }
    ...
    // For client-side connections, we need both token and roomId for proper Hathora handshake
    // Since we don't have a direct token in this path, we need to get it from server info
    console.log('🌍 Creating direct WebSocket connection to Hathora server')
    
    if (finalRoomId) {
      // Use direct connection with room ID and check if server has token
      const serverToken = serverInfo.token || serverInfo.connectionToken
      if (serverToken) {
        socketUrl = `wss://${serverInfo.host}:${serverInfo.port}?token=${encodeURIComponent(serverToken)}&roomId=${encodeURIComponent(finalRoomId)}`
        console.log('🌍 Connecting to Hathora server with token and room ID parameters')
      } else {
        socketUrl = `wss://${serverInfo.host}:${serverInfo.port}?roomId=${encodeURIComponent(finalRoomId)}`
        console.log('🌍 Connecting to Hathora server with room ID parameter (no token available)')
      }
    } else {
      // Fallback to direct connection without room ID
      socketUrl = `wss://${serverInfo.host}:${serverInfo.port}`
      console.log('🌍 Connecting to Hathora server with direct connection (no parameters)')
      console.warn('⚠️ Missing roomId - WebSocket handshake may fail')
    }
