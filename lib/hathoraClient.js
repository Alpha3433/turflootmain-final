...
class TurfLootHathoraClient {
  ...
  async connectToGame(gameConfig = {}) {
    ...
    // FIXED: For global-practice-bots, skip Hathora room creation and connect directly to our local server
    // This ensures all users join the same shared game instance
    if (requestedRoomId === 'global-practice-bots') {
      console.log('🌍 Global Multiplayer detected - connecting to shared server instance')
      
      // Import Socket.IO client
      const { io } = await import('socket.io-client')
      
      // Connect to our local server which manages the global practice room
      ...
      this.currentConnection = {
        ...
        serverInfo: {
          ...
          port: 3000,
          isLocal: false,
          region: 'US-East-1', // Primary region for global multiplayer
          roomId: requestedRoomId
        },
        roomId: requestedRoomId
      }

      console.log('✅ Global Multiplayer connection established:', {
        socketUrl,
        roomId: requestedRoomId,
        serverRegion: 'US-East-1 (Global)'
      })
