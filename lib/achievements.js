...
// Check and award new achievements after game completion
export async function checkAndAwardAchievements(userId, userData) {
  ...
  for (const achievement of achievements) {
    ...
    switch (req.type) {
      case 'daily_streak':
        currentValue = userData?.achievements?.dailyStreak || 0
        meetsRequirement = currentValue >= req.value
        break
      case 'games_played':
        currentValue = stats.games_played || 0
        meetsRequirement = currentValue >= req.value
    ...
    if (meetsRequirement) {
      ...
      totalAPAwarded += achievement.ap
    }
  }
  
  // Update database with new achievements and AP
  if (newlyCompleted.length > 0) {
    ...
    await users.updateOne(
      { id: userId },
      {
        $inc: { 'achievements.totalAP': totalAPAwarded },
        $push: { 
          'achievements.completedAchievements': { 
            $each: newlyCompleted.map(a => a.id) 
          }
        }
      }
    )
    
    console.log(`🏆 Awarded ${newlyCompleted.length} achievements to ${userId}:`, newlyCompleted.map(a => a.name))
    console.log(`💎 Total AP awarded: ${totalAPAwarded}`)
  }
  
  return newlyCompleted
}

// Helper function to get database connection (assuming it exists in the API context)
async function getDb() {
  // This will be called from the API context where getDb is available
  // We'll need to import this when needed
  throw new Error('getDb must be called from API context')
}