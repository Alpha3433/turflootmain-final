  ...
  try {
    if (typeof signAndSendTransaction === 'function') {
      ...
      if (resolved.signingWallet) {
        // Always forward the resolved wallet so Privy can locate the proper signer
      ...
      const result = await signAndSendTransaction(request)
      signature = normaliseSignature(result)
    }
  } catch (error) {
    primaryError = error
    console.warn('⚠️ Privyy signAndSendTransaction failed, falling back to direct wallet methods:', error)
  }

  if (!signature && resolved.signingWallet && typeof resolved.signingWallet.signAndSendTransaction === 'function') {
    try {
      ...
      console.warn('⚠️ Direct wallet signAndSendTransaction failed:', error)
    }
  }

  if (!signature && resolved.signingWallet && typeof resolved.signingWallet.sendTransaction === 'function') {
    try {
      const result = await resolved.signingWallet.sendTransaction(transaction, connection, {
        skipPreflight: false
      })
      signature = normaliseSignature(result)
    } catch (error) {
      ...
      console.warn('⚠️ Direct wallet sendTransaction failed:', error)
    }
  }

  if (!signature && typeof signTransaction === 'function') {
    try {
      ...
      console.warn('⚠️ signTransaction fallback failed:', error)
    }
  }

  if (!signature) {
    const failureError = primaryError || new Error('No Privy transaction signing method available.')
    console.error('❌ Privyy signing failed:', failureError)
