...
export const executePrivyyArenaEntry = async ({
  ...
  try {
    const signingWallet = resolved.signingWallet
    const hasPrivySignAndSend = typeof signAndSendTransaction === 'function'

    if (hasPrivySignAndSend && signingWallet) {
      ...
      signingWallet &&
      typeof signingWallet.signAndSendTransaction === 'function'
    ) {
      const result = await signingWallet.signAndSendTransaction(transaction, {
        chain: targetChain,
        sendOptions: { skipPreflight: false }
      })
      signature = normaliseSignature(result)
    } else if (typeof signTransaction === 'function' && signingWallet) {
      ...
      if (!rawTx) {
        throw new Error('Signed transaction payload missing from Privy response.')
      }
      const rawSignature = await connection.sendRawTransaction(rawTx, {
        skipPreflight: false
      })
      ...
      return {
        success: false,
        error:
          'No Privy transaction signing method available. Connect a Solana wallet that can sign transactions.'
      }
    }
  } catch (error) {
