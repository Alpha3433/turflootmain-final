...
const walletSupportsDirectSend = (wallet) => {
  if (!wallet) {
    return false
  }
  ...
  try {
    if (typeof signAndSendTransaction === 'function') {
      ...
      if (resolved.signingWallet) {
        if (walletSupportsDirectSend(resolved.signingWallet)) {
        ...
        const walletIdentifier =
          resolved.signingWallet.id || resolved.signingWallet.walletId || resolved.signingWallet?.metadata?.id
      ...
      resolved.signingWallet &&
      typeof resolved.signingWallet.sendTransaction === 'function'
    ) {
