...
let hasWarnedAboutUnsupportedNetwork = false;

const warnUnsupportedNetwork = (network) => {
  if (hasWarnedAboutUnsupportedNetwork) {
    return;
  }

  hasWarnedAboutUnsupportedNetwork = true;
  console.warn(
    `⚠️ Unsupported Solana network "${network}" configured. Turfloot paid rooms require mainnet; defaulting to ${DEFAULT_SOLANA_NETWORK}.`
  );
};

export const normaliseSolanaNetwork = (network) => {
  ...
  if (candidate.startsWith('solana:')) {
    return normaliseSolanaNetwork(candidate.split(':').slice(1).join(':'));
  }
  ...
  warnUnsupportedNetwork(candidate);
  return DEFAULT_SOLANA_NETWORK;
};

const SOLANA_NETWORK = normaliseSolanaNetwork(
...
export const getConfiguredSolanaNetwork = () => SOLANA_NETWORK;

const normaliseNetworkForHelius = (network) => {
  const normalized = normaliseSolanaNetwork(network);
  if (normalized === 'mainnet-beta') {
    return 'mainnet';
  }
  return normalized;
};
