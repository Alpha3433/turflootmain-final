// User authentication and session management - Simplified version
...
// Simple JWT token generation (simplified without jsonwebtoken dependency)
function generateSimpleToken(payload) {
  const header = btoa(JSON.stringify({ alg: 'HS256', typ: 'JWT' }))
  const data = btoa(JSON.stringify({ ...payload, exp: Date.now() + 7 * 24 * 60 * 60 * 1000 }))
  const signature = btoa(`${header}.${data}.${JWT_SECRET}`) // Simplified signature
  return `${header}.${data}.${signature}`
}

// Simple password hashing (simplified without bcryptjs)
