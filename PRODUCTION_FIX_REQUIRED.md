...
- **External API Status**: ❌ Failing (`https://agario-multiplayer.preview.emergentagent.com/api/ping` → 502 Bad Gateway)
...
curl https://agario-multiplayer.preview.emergentagent.com/api/ping

# Should allow name saving:
curl -X POST https://agario-multiplayer.preview.emergentagent.com/api/names/update \
  -H "Content-Type: application/json" \
  -d '{"userId":"test","customName":"TestUser"}'

# Should allow friend requests:  
curl -X POST https://agario-multiplayer.preview.emergentagent.com/api/friends/send-request \
  -H "Content-Type: application/json" \
