...
export default function DebugPrivyClient() {
  ...
  useEffect(() => {
    ...
    // Test Privy services connectivity (client-only)
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-gray-800 p-6 rounded-lg">
          ...
          <div className="space-y-3">
            <button onClick={handleTestLogin} disabled={!ready} className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-gray-600 disabled:opacity-50 text-white px-4 py-2 rounded-lg transition-colors">
              {ready ? 'Test Login' : 'Waiting for Privy...'}
            </button>

            {authenticated && (
              <button onClick={logout} className="w-full bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg transition-colors">
                Logout
              </button>
            )}

            <button onClick={() => window.location.reload()} className="w-full bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-lg transition-colors">
              Reload Page
            </button>
          </div>
        ...
        <div className="bg-gray-800 p-6 rounded-lg">
          ...
          <pre className="text-xs bg-gray-900 p-3 rounded overflow-auto">{JSON.stringify(debugInfo, null, 2)}</pre>
        </div>
      </div>

      <div className="mt-8 text-center">
        <a href="/" className="bg-purple-600 hover:bg-purple-700 px-6 py-3 rounded-lg text-white font-medium">
          ← Back to TurfLoot
        </a>
      </div>
    </div>
