...
- **Health Check**: `https://turf-fixer.preview.emergentagent.com/health` → Returns `{"status":"healthy"}`
- **Name Saving**: `https://turf-fixer.preview.emergentagent.com/names-api/update` → Successfully saves names
- **Name Retrieval**: `https://turf-fixer.preview.emergentagent.com/names-api/get?userId=X` → Successfully retrieves names
...
# Save name:
curl -X POST "https://turf-fixer.preview.emergentagent.com/names-api/update" \
  -H "Content-Type: application/json" \
  -d '{"userId": "'$USER_ID'", "customName": "TestBypassUser"}'
# Result: {"success":true,"message":"Name saved successfully via bypass route"}

# Retrieve name:
curl "https://turf-fixer.preview.emergentagent.com/names-api/get?userId=$USER_ID"  
