...
- **Health Check**: `https://multiplayer-fix-1.preview.emergentagent.com/health` → Returns `{"status":"healthy"}`
- **Name Saving**: `https://multiplayer-fix-1.preview.emergentagent.com/names-api/update` → Successfully saves names
- **Name Retrieval**: `https://multiplayer-fix-1.preview.emergentagent.com/names-api/get?userId=X` → Successfully retrieves names
...
# Save name:
curl -X POST "https://multiplayer-fix-1.preview.emergentagent.com/names-api/update" \
  -H "Content-Type: application/json" \
  -d '{"userId": "'$USER_ID'", "customName": "TestBypassUser"}'
# Result: {"success":true,"message":"Name saved successfully via bypass route"}

# Retrieve name:
curl "https://multiplayer-fix-1.preview.emergentagent.com/names-api/get?userId=$USER_ID"  
